...
exports.order = function order(req, res, next) {
  ...
  var categories = getCategoriesByProducts(order.names)
  var voucher = getVoucherByCategory(categories);
  ...
  if (reductions[order.reduction]) {
    ...
    returnValue = {total: parseFloat(total.toFixed(2), 10), voucher: 0, licenses: getLicence(categories, order.country)};
...
function getLicence(categories, country) {
  if (!categories) return [];
  var idLicence = 0, nbLicences = licencesDatas.length, licenses = [];
  for (; idLicence < nbLicences; idLicence++) {
    var idCategory = 0, nbCategories = categories.length;
    for (; idCategory < nbCategories; idCategory++) {
      if (categories[idCategory] === licencesDatas[idLicence].category && country === licencesDatas[idLicence].country) {}
        licenses.push(licencesDatas[idLicence].license);
    }
  }
  licenses = arrayUnique(licenses);
  return licenses;
}
...
function getCategoriesByProducts(products) {
  ...
  for(; index < products.length ; index++) {
    categories.push(getCategoryByProduct(products[index]));
  }
  return categories;
}
...
function getCategoryByProduct(product) {
  ...
  for(; index < categories.length; index++) {
    if(categories[index].item == product) {
      return categories[index].category;
    }
  }
}

function getVoucherByCategory(category) {
  var voucher = 0;
  var index = 0;
  for(; index < voucherData.length; index++) {
    if(voucherData[index].category == category && voucherData[index].amount > voucher) {
      voucher = voucherData[index].amount;
    }
  }
  return voucher;
}
