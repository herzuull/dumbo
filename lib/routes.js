...
exports.order = function order(req, res, next) {
  ...
  if (reductions[order.reduction]) {
    ...
    returnValue = {total: parseFloat(total.toFixed(2), 10), voucher: 0, licenses: getLicence(order.category, order.country)};
...
function getLicence(category, country) {
  var license = licencesDatas.filter(function(item) {
    return category === item.category && country === item.country;
  });
  return license;
}
